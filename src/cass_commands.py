...
class CassCommands:
    ...
    def get_serial_ports(self):
        """Cross-platform method to find logger serial ports."""
        ...
        if len(logger_ports) != 2:
            ...
            if system == "windows":
                ...
                print(
                    "3. Use list_available_ports() and set_manual_serial_ports() if needed"
                )
                print("4. Make sure no other applications are using the COM ports")

            return None
        else:
            print(f"Found logger ports: {logger_ports}")
            return logger_ports
    ...
    def diagnose_windows_ports(self):
        ...
        if len(teensy_candidates) == 2:
            ...
            print(
                f"Found {len(teensy_candidates)} potential devices, but need exactly 2."
            )
        else:
    ...
    def _delete_file(self, filename):
        ...
        filename_term = (
            filename + "x"
        )  # append terminator so firmware knows filename is complete
